...
if ($_FILES['image']['name'] != "" && $_FILES['image1']['name'] != "") {
	...
	$query = "UPDATE products SET groom = ?, bride = ?, description = ?, description1 = ?,date = ?, image = ?, image1 = ? WHERE product_id = ?";
	$stmt = $cn->prepare($query);
	$stmt->bind_param('sssssssi', $groom, $bride, $description,$description1,$date, $img_path ,$img1_path,$id);
	$stmt->execute();
	$stmt->close();
    $cn->close();
